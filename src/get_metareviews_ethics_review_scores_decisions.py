...
for submission in submissions:
    ...
    for reply in submission.details['replies']:
        ...
        elif f'{VENUE_ID}/{submission_name}{submission.number}/-/{decision_name}' in reply['invitations']:
            decision_note = openreview.api.Note.from_json(reply)
            decision = decision_note.content['decision']['value']
            has_decision = True
    ...
    if not has_decision:
        submission_info.insert(5, 'None')
    else:
        submission_info.insert(5, decision)
...
with open(f'../data/reviews-{timestamp}.csv', 'w') as outfile:
    ...
    csvwriter.writerow(['submission_id', 'title', 'AC_recommendation', 'AC_confidence', 'ethics_reviews', 'decision',
                        'review_rating1',
