...
rating_counts = df_filtered['average_rating'].value_counts().reset_index()
rating_counts.columns = ['average_rating', 'frequency']

# Sort the DataFrame by average_rating
rating_counts = rating_counts.sort_values(by='average_rating')

# Step 4: Calculate accept and reject counts for each average rating
accept_counts = df_filtered[df_filtered['decision'] == 'Accept'].groupby('average_rating').size().reindex(rating_counts['average_rating'], fill_value=0)
reject_counts = df_filtered[df_filtered['decision'] == 'Reject'].groupby('average_rating').size().reindex(rating_counts['average_rating'], fill_value=0)

# Format average_rating to two decimal places
rating_counts['average_rating'] = rating_counts['average_rating'].round(2)
...
fig.add_trace(go.Bar(
    x=rating_counts['average_rating'].astype(str),
    y=accept_counts,
    name='Accept',
...
fig.update_layout(
    ...
    showlegend=True,
    title=dict(
        text='Frequency Distribution of Average Review Ratings and Decisions',
...
# Show the plot
fig.show()
fig.write_image(f"../data/figures/decision_vs_avg_review_scores.png", scale=6, width=1920, height=1080)
